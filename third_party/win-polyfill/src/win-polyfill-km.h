...
} RTL_USER_PROCESS_PARAMETERS_VISTA, *PRTL_USER_PROCESS_PARAMETERS_VISTA,
    *PPROCESS_PARAMETERS_VISTA;
...
typedef _Function_class_(RTL_HEAP_COMMIT_ROUTINE) _IRQL_requires_same_ NTSTATUS NTAPI
...
typedef struct _FILE_RENAME_INFORMATION
{
    ...
    ULONG FileNameLength;
    WCHAR FileName[1];
} FILE_RENAME_INFORMATION, *PFILE_RENAME_INFORMATION, FILE_LINK_INFORMATION,
...
EXTERN_C
__kernel_entry NTSYSCALLAPI NTSTATUS NTAPI NtQueryDirectoryFile(
    _In_ HANDLE FileHandle,
    _In_opt_ HANDLE Event,
...
EXTERN_C
NTSYSAPI NTSTATUS WINAPI ZwClose(_In_ HANDLE Handle);
...
IoVolumeDeviceToDosName(_In_ PVOID VolumeDeviceObject, _Out_ PUNICODE_STRING DosName);

EXTERN_C
NTSYSAPI NTSTATUS NTAPI RtlpEnsureBufferSize(ULONG flags, PRTL_BUFFER pBuffer, SIZE_T requiredSize);
...
typedef enum _PROCESSINFOCLASS
{
    ...
    ProcessImageFileMapping,   // buffer is a pointer to a file handle open with SYNCHRONIZE |
                             // FILE_EXECUTE access, return value is whether the handle is the same
    ...
    ProcessWindowInformation,  // returns the windowflags and windowtitle members of the process'
                               // peb->rtl_user_process_params
    MaxProcessInfoClass
} PROCESSINFOCLASS;
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI RtlHashUnicodeString(
...
ULONG
NTAPI
RtlGetFullPathName_U(const WCHAR *name, ULONG size, WCHAR *buffer, WCHAR **file_part);
...
NTSTATUS
NTAPI
NtTerminateThread(IN HANDLE ThreadHandle, IN NTSTATUS ExitStatus);
...
NTSTATUS
NTAPI
NtResumeThread(IN HANDLE ThreadHandle, OUT PULONG SuspendCount OPTIONAL);

EXTERN_C
NTSYSAPI
NTSTATUS
NTAPI
NtTerminateProcess(IN HANDLE ProcessHandle, IN NTSTATUS ExitStatus);
...
SIZE_T
NTAPI
RtlCompactHeap(IN HANDLE hHeap, IN ULONG flags);

EXTERN_C
NTSYSAPI
PVOID
NTAPI
RtlDestroyHeap(IN HANDLE hHeap);

EXTERN_C
NTSYSAPI
VOID NTAPI RtlEnterCriticalSection(IN LPCRITICAL_SECTION pSection);

EXTERN_C
NTSYSAPI
VOID NTAPI RtlLeaveCriticalSection(IN LPCRITICAL_SECTION pSection);

EXTERN_C
NTSYSAPI
VOID NTAPI RtlDeleteCriticalSection(IN LPCRITICAL_SECTION pSection);
...
EXTERN_C
NTSYSAPI
VOID NTAPI RtlSetLastWin32Error(IN ULONG error);
...
NTSTATUS
NTAPI
RtlDestroyProcessParameters(IN PPROCESS_PARAMETERS_VISTA ProcessParameters);
...
PIMAGE_NT_HEADERS
NTAPI
RtlImageNtHeader(__in PVOID baseAddress);
...
NTSTATUS
NTAPI
NtUnmapViewOfSection(__in HANDLE ProcessHandle, __in_opt PVOID BaseAddress);
...
typedef __drv_sameIRQL
    __drv_functionClass(RTL_AVL_COMPARE_ROUTINE) RTL_GENERIC_COMPARE_RESULTS NTAPI
...
typedef __drv_sameIRQL __drv_functionClass(RTL_AVL_ALLOCATE_ROUTINE)
    __drv_allocatesMem(Mem) PVOID NTAPI
    RTL_AVL_ALLOCATE_ROUTINE(__in struct _RTL_AVL_TABLE *Table, __in LONG ByteSize);
...
typedef __drv_sameIRQL __drv_functionClass(RTL_AVL_FREE_ROUTINE) VOID NTAPI RTL_AVL_FREE_ROUTINE(
    __in struct _RTL_AVL_TABLE *Table,
...
typedef __drv_sameIRQL __drv_functionClass(RTL_AVL_MATCH_FUNCTION) NTSTATUS NTAPI
...
BOOLEAN
NTAPI
RtlDeleteElementGenericTableAvl(__in PRTL_AVL_TABLE Table, __in PVOID Buffer);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlLookupElementGenericTableAvl(__in PRTL_AVL_TABLE Table, __in PVOID Buffer);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlEnumerateGenericTableAvl(__in PRTL_AVL_TABLE Table, __in BOOLEAN Restart);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlEnumerateGenericTableWithoutSplayingAvl(__in PRTL_AVL_TABLE Table, __inout PVOID *RestartKey);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI RtlLookupFirstMatchingElementGenericTableAvl(
    __in PRTL_AVL_TABLE Table,
    __in PVOID Buffer,
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI RtlEnumerateGenericTableLikeADirectory(
    __in PRTL_AVL_TABLE Table,
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlGetElementGenericTableAvl(__in PRTL_AVL_TABLE Table, __in ULONG I);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
ULONG
NTAPI
RtlNumberGenericTableElementsAvl(__in PRTL_AVL_TABLE Table);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
#if (NTDDI_VERSION >= NTDDI_WINXP)
EXTERN_C
__checkReturn NTSYSAPI BOOLEAN NTAPI RtlIsGenericTableEmptyAvl(__in PRTL_AVL_TABLE Table);
#endif // NTDDI_VERSION >= NTDDI_WINXP
...
typedef RTL_GENERIC_COMPARE_RESULTS NTAPI RTL_GENERIC_COMPARE_ROUTINE(
...
typedef PVOID NTAPI
RTL_GENERIC_ALLOCATE_ROUTINE(__in struct _RTL_GENERIC_TABLE *Table, __in LONG ByteSize);
...
typedef VOID NTAPI
RTL_GENERIC_FREE_ROUTINE(__in struct _RTL_GENERIC_TABLE *Table, __in PVOID Buffer);
...
EXTERN_C NTSYSAPI VOID NTAPI RtlInitializeGenericTable(
...
BOOLEAN
NTAPI
RtlDeleteElementGenericTable(__in PRTL_GENERIC_TABLE Table, __in PVOID Buffer);
#endif
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlLookupElementGenericTable(__in PRTL_GENERIC_TABLE Table, __in PVOID Buffer);
#endif
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlEnumerateGenericTable(__in PRTL_GENERIC_TABLE Table, __in BOOLEAN Restart);
#endif
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlEnumerateGenericTableWithoutSplaying(__in PRTL_GENERIC_TABLE Table, __inout PVOID *RestartKey);
#endif
...
EXTERN_C
__checkReturn NTSYSAPI PVOID NTAPI
RtlGetElementGenericTable(__in PRTL_GENERIC_TABLE Table, __in ULONG I);
#endif
...
ULONG
NTAPI
RtlNumberGenericTableElements(__in PRTL_GENERIC_TABLE Table);
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__checkReturn NTSYSAPI BOOLEAN NTAPI RtlIsGenericTableEmpty(__in PRTL_GENERIC_TABLE Table);
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtCreateKey(
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NtCreateKeyTransacted(
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtOpenKey(
    __out PHANDLE KeyHandle,
    __in ACCESS_MASK DesiredAccess,
    __in POBJECT_ATTRIBUTES ObjectAttributes);
#endif
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtOpenKeyEx(
    __out PHANDLE KeyHandle,
    __in ACCESS_MASK DesiredAccess,
    __in POBJECT_ATTRIBUTES ObjectAttributes,
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtOpenKeyTransacted(
    __out PHANDLE KeyHandle,
    __in ACCESS_MASK DesiredAccess,
    __in POBJECT_ATTRIBUTES ObjectAttributes,
    __in HANDLE TransactionHandle);
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtOpenKeyTransactedEx(
    __out PHANDLE KeyHandle,
    __in ACCESS_MASK DesiredAccess,
    __in POBJECT_ATTRIBUTES ObjectAttributes,
    __in ULONG OpenOptions,
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtDeleteKey(__in HANDLE KeyHandle);
#endif

#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI
    NtDeleteValueKey(__in HANDLE KeyHandle, __in PUNICODE_STRING ValueName);
#endif

#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) __drv_when(Length == 0, __drv_valueIs(< 0))
    __drv_when(Length > 0, __drv_valueIs(< 0; == 0)) NTSYSAPI NTSTATUS NTAPI NtEnumerateKey(
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) __drv_when(Length == 0, __drv_valueIs(< 0))
    __drv_when(Length > 0, __drv_valueIs(< 0; == 0)) NTSYSAPI NTSTATUS NTAPI NtEnumerateValueKey(
        __in HANDLE KeyHandle,
        __in ULONG Index,
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtFlushKey(__in HANDLE KeyHandle);
#endif

#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtNotifyChangeMultipleKeys(
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtQueryMultipleValueKey(
    __in HANDLE KeyHandle,
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) __drv_when(Length == 0, __drv_valueIs(< 0))
    __drv_when(Length > 0, __drv_valueIs(< 0; == 0)) NTSYSAPI NTSTATUS NTAPI NtQueryKey(
        __in HANDLE KeyHandle,
        __in KEY_INFORMATION_CLASS KeyInformationClass,
        __out_bcount_opt(Length) PVOID KeyInformation,
        __in ULONG Length,
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) __drv_when(Length == 0, __drv_valueIs(< 0))
    __drv_when(Length > 0, __drv_valueIs(< 0; == 0)) NTSYSAPI NTSTATUS NTAPI NtQueryValueKey(
        __in HANDLE KeyHandle,
...
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI
    NtRenameKey(__in HANDLE KeyHandle, __in PUNICODE_STRING NewName);
#endif

#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtSetInformationKey(
    __in HANDLE KeyHandle,
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__drv_maxIRQL(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI NtSetValueKey(
    __in HANDLE KeyHandle,
    __in PUNICODE_STRING ValueName,
...
EXTERN_C NTSYSAPI NTSTATUS NTAPI RtlCreateUserThread(
    _In_ HANDLE processHandle,
...
EXTERN_C NTSYSAPI VOID NTAPI RtlExitUserThread(_In_ NTSTATUS Status);

EXTERN_C NTSYSAPI DWORD NTAPI RtlComputeCrc32(DWORD dwInitial, const BYTE *pData, INT iLen);

EXTERN_C NTSYSAPI NTSTATUS NTAPI NtUnloadKey(IN POBJECT_ATTRIBUTES DestinationKeyName);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
NtUnloadKey2(IN POBJECT_ATTRIBUTES KeyObjectAttributes, IN BOOLEAN ForceUnload);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
NtUnloadKeyEx(IN POBJECT_ATTRIBUTES KeyObjectAttributes, IN HANDLE EventHandle OPTIONAL);
...
EXTERN_C NTSYSAPI NTSTATUS NTAPI ZwQueryBootEntryOrder(OUT PULONG Ids, IN OUT PULONG Count);
...
EXTERN_C NTSYSAPI NTSTATUS NTAPI
ZwQueryBootOptions(OUT PBOOT_OPTIONS BootOptions, IN OUT PULONG BootOptionsLength);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
ZwSetBootOptions(IN PBOOT_OPTIONS BootOptions, IN ULONG FieldsToChange);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
ZwEnumerateBootEntries(__out_bcount_opt(*BufferLength) PVOID Buffer, __inout PULONG BufferLength);

EXTERN_C NTSYSAPI NTSTATUS NTAPI ZwAddBootEntry(__in PBOOT_ENTRY BootEntry, __out_opt PULONG Id);

EXTERN_C NTSYSAPI NTSTATUS NTAPI ZwModifyBootEntry(__in PBOOT_ENTRY BootEntry);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
ZwSetBootEntryOrder(__in_ecount(Count) PULONG Ids, __in ULONG Count);
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
EXTERN_C
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwOpenKeyEx(
    _Out_ PHANDLE KeyHandle,
    _In_ ACCESS_MASK DesiredAccess,
    _In_ POBJECT_ATTRIBUTES ObjectAttributes,
    _In_ ULONG OpenOptions);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _When_(Length == 0, _Post_satisfies_(return < 0))
    _When_(Length > 0, _Post_satisfies_(return <= 0)) _Success_(return == STATUS_SUCCESS)
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwSetValueKey(
    _In_ HANDLE KeyHandle,
    _In_ PUNICODE_STRING ValueName,
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwQueryInformationFile(
    _In_ HANDLE FileHandle,
    _Out_ PIO_STATUS_BLOCK IoStatusBlock,
    _Out_writes_bytes_(Length) PVOID FileInformation,
    _In_ ULONG Length,
...
EXTERN_C NTSYSAPI NTSTATUS NTAPI
NtLoadKey(IN POBJECT_ATTRIBUTES DestinationKeyName, IN POBJECT_ATTRIBUTES HiveFileName);

EXTERN_C NTSYSAPI NTSTATUS NTAPI
ZwLoadKey(IN POBJECT_ATTRIBUTES DestinationKeyName, IN POBJECT_ATTRIBUTES HiveFileName);
...
#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI ZwWriteFile(
    _In_ HANDLE FileHandle,
    _In_opt_ HANDLE Event,
    _In_opt_ PIO_APC_ROUTINE ApcRoutine,
    _In_opt_ PVOID ApcContext,
...
EXTERN_C
_IRQL_requires_max_(PASSIVE_LEVEL) NTSYSAPI NTSTATUS NTAPI
    ZwTerminateProcess(_In_opt_ HANDLE ProcessHandle, _In_ NTSTATUS ExitStatus);

#if (NTDDI_VERSION >= NTDDI_WIN2K)
EXTERN_C
__kernel_entry NTSYSCALLAPI NTSTATUS NTAPI NtDeviceIoControlFile(
    _In_ HANDLE FileHandle,
    _In_opt_ HANDLE Event,
    _In_opt_ PIO_APC_ROUTINE ApcRoutine,
    _In_opt_ PVOID ApcContext,
...
EXTERN_C DECLSPEC_IMPORT void __stdcall MD5Update(
    MD5_CTX *context,
    unsigned char *input,
    unsigned int inlen);
...
NTSTATUS
NTAPI
LdrAddRefDll(IN ULONG Flags, IN PVOID BaseAddress);
